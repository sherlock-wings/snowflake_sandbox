...
def chunk_check(schema_input: dict, filesize_limit_mb: int = 300, dict_input: dict=None, dataframe_input: pd.DataFrame=pd.DataFrame(), callout_size: bool=False) -> Tuple[pd.DataFrame, dict]:
    ...
    if callout_size:
        logging.info(f"Current calculated space of df is {size:,.2f} MB")
    if size >= filesize_limit_mb:
        logging.info("SIZE LIMIT TRIGGERED")
    ...
    while pages_remain and not watermark_crossed:
        ...
        while retry_count <= max_retries:
            try:
                ...
                logging.info(f"Call for page {page_num:,} of user @{bsky_username}'s post data was blocked by Rate-Limiting.")
                logging.info(f"Trying again in {wait_period_seconds:,} seconds (attempt {retry_count} of {max_retries})...")
        ...
        logging.info(f"Ingesting {page_num:,} pages of post-data from user @{bsky_username}...", end='\r')
        #
        ...
        for item in feed:
            ...
            if ts <= watermark_ts:
                ...
                logging.info(f"Encountered post creation timestamp is {datetime.strftime(ts, '%Y-%m-%d %H:%M:%S.%f %z')}, latest known timestamp for this user is {datetime.strftime(watermark_ts, '%Y-%m-%d %H:%M:%S.%f %z')}")
                logging.info("Ingestion for this user will now stop.\n")
                break
...
def upload_file_to_azr(file_to_upload: str) -> None:
    ...
    with open(file_to_upload, "rb") as data:
        ...
        logging.info(f"Uploaded file: {blob_name}")
...
def clear_local_dir() -> None:
    ...
    for file in local_files:
        if file in azr_files:
            ...
            logging.info(f"File {file} detected locally but not detected in Azure Storage account!!\nYou may have some local data missing from the cloud. Consider reuploading.")
...
def write_watermark_table() -> None: 
    ...
    if len(azr_files) == 0:
        ...
        logging.info("This means incremental ingestion will not be applied. If that is unexpected, then this run may be ingesting duplicate records.")
        logging.info("If you don't want that, cancel this ingestion now with CTRL+C!\n")
    ...
    logging.info(f"{len(azr_files):,} files with max date {max_date} detected in Azure Cloud Storage.\nDownloading files to generate watermark table...")
    ...
    for i in range(len(azr_files)):
        ...
        logging.info(f"{i+1} of {len(azr_files)} max-date files downloaded from Azure")


    df = df.groupby('post_author_did')['post_created_timestamp'].max().reset_index()
    logging.info("Writing control table...")
    df.to_csv(f"{WTM_TBL_DIR}/extract_feed_control_tbl.csv", index=False)
    logging.info(f"High-Watermark Control table written to {WTM_TBL_DIR}/extract_feed_control_tbl.csv\n")
...
# decorator to run driver function on schedule
...
def extract_feed(myTimer: func.TimerRequest) -> None:
    ...
    logging.info(f"New BlueSky feed-extract session opened at {datetime.now(pytz.timezone("America/New_York")).strftime('%Y-%m-%d %H:%M:%S %Z')}")
    logging.info(f"Logging in as BlueSky User {USR}... \nLET'S GET THIS DATA! ( ͡⌐■ ͜ʖ ͡-■)\n\n")
    ...
    logging.info(f"Detected {len(following_users):,} BlueSky Users being followed by user @{session_usr}")
    logging.info(f"Parsing posts...")
    df = None
    c = 0
    ...
    logging.info(f"{len(files)} total CSV files detected.")
    ...
    clear_local_dir()
    # from Azure Function App Template vv
    if myTimer.past_due:
