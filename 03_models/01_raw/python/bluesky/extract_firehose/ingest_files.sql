...
create table if not exists firehose_processed (
 utc_post_created_at_timestamp timestamp_tz(9)
,content_id varchar
...
with init as (
select value
      ,cast(trim(value:commit:record:createdAt, '"') as timestamp_tz) as utc_post_created_at_timestamp
...
where cast(trim(value:commit:record:createdAt, '"') as timestamp_tz) > 
      nvl(
          (select max(utc_post_created_at_timestamp) from firehose_processed)
...
order by utc_post_created_at_timestamp desc 
limit 500;