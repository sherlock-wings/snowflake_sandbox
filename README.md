...
This is never done manually. It is done by using a stored procedure called `CLONE_TO_SANDBOX`. The sproc works by cloning from a target environment (specified by the caller) and into the Sandbox. The sproc knows automatically to generate the schemas as clones, using the correct name. Also, it correctly "rebuilds" the RBAC so the schema is is usable as expected post-execution. Finally, every `CLONE`, `GRANT` and `REVOKE` statement ran by that sproc is stored in a view called `CLONE_TO_SANDBOX_CALL_LOG_VW`, meaning all the work done by the sproc is totally traceable. 
